...
{
	struct_xbU3PData vData = pxbOrder->DataList[0];

	pxbOrder->ParserOrder(vData.CaseID);
	doPrepare_U3p(vData.CalName, vData.CaseID, vData.PdPercent);
...
void CxbSolveMvc::Run()
{
	...
	//
	for each (struct_xbU3PData vData in pxbOrder->DataList)
	{
...
void CxbSolveMvc::doPrepare_U3p(string  vCalName,string vCaseID,double vPdPersent)
{
